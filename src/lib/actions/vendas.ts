...
// Junta 2+ vendas (lotes) numa "cobrança conjunta" — cada venda mantém seus
// próprios animais/rateio, só passam a aparecer juntas com total somado.
export async function agruparVendasCobranca(localId: string, vendaIds: string[]) {
  const supabase = await criarClienteServidor();
  const { error } = await supabase.rpc("agrupar_vendas_cobranca", { p_venda_ids: vendaIds });
  if (error) throw new Error(error.message);
  revalidatePath(caminho(localId));
}

// Tira uma venda da cobrança conjunta (se sobrar só 1, o grupo inteiro se
// desfaz — a RPC cuida disso).
export async function desagruparVendaCobranca(localId: string, vendaId: string) {
  const supabase = await criarClienteServidor();
  const { error } = await supabase.rpc("desagrupar_venda_cobranca", { p_venda_id: vendaId });
  if (error) throw new Error(error.message);
  revalidatePath(caminho(localId));
}

// Marca/desmarca pago, entregue, contrato assinado e posse transferida de
// UMA égua vendida — a RPC grava a data de hoje sozinha quando o campo
// passa a true, e limpa quando volta a false.
export async function atualizarStatusVendaAnimal(
  localId: string,
  animalId: string,
  status: { paga: boolean; entregue: boolean; contratoAssinado: boolean; posseTransferida: boolean },
) {
  const supabase = await criarClienteServidor();
  const { error } = await supabase.rpc("atualizar_status_venda_animal", {
    p_animal_id: animalId,
    p_paga: status.paga,
    p_entregue: status.entregue,
    p_contrato_assinado: status.contratoAssinado,
    p_posse_transferida: status.posseTransferida,
  });
  if (error) throw new Error(error.message);
  revalidatePath(caminho(localId));
}

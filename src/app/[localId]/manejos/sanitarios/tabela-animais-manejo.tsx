...
import { Stethoscope, ArrowUp, ArrowDown, ArrowUpDown } from "lucide-react";
...
import { registrarManejoSanitario } from "@/lib/actions/manejos-sanitarios";
import { CampoMedicamentos } from "./campo-medicamentos";
import type { AnimalManejo, Especie, OpcaoArea, OpcaoMedicamento } from "./page";
...
function valorOrdenacao(a: AnimalManejo, coluna: ColunaId): string | number {
  switch (coluna) {
    case "identificacao":
...
export function TabelaAnimaisManejo({ localId, animais, areas, medicamentos, podeEditar }: Props) {
  ...
  const [medicamentosSelecionados, setMedicamentosSelecionados] = useState<Set<string>>(new Set());
  const [areaManejoId, setAreaManejoId] = useState("");
  ...
  async function confirmarManejo() {
    setEmAndamento(true);
    setErro(undefined);
    const formData = new FormData();
    formData.set("localId", localId);
    formData.set("animalIds", JSON.stringify(Array.from(selecionados)));
    formData.set("medicamentoIds", JSON.stringify(Array.from(medicamentosSelecionados)));
    formData.set("areaId", areaManejoId);
    formData.set("especie", especieSelecionada);
    formData.set("data", data);
    formData.set("responsavel", responsavel);
    formData.set("descricao", descricao);
    formData.set("observacoes", observacoes);

    const resultado = await registrarManejoSanitario(undefined, formData);
    setEmAndamento(false);
    if (resultado?.erro) {
      setErro(resultado.erro);
      return;
    }
    toast.success(`Manejo sanitário registrado para ${selecionados.size} animal(is).`);
    setSelecionados(new Set());
    setMedicamentosSelecionados(new Set());
    setResponsavel("");
    setDescricao("");
    setObservacoes("");
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold">Manejos Sanitários</h1>
          <p className="text-sm text-muted-foreground">
            Marque os animais e registre os medicamentos aplicados neles.
          </p>
        </div>
        ...
        <div className="sticky bottom-0 z-10 -mx-6 border-t bg-background/95 px-6 py-3 shadow-[0_-4px_12px_rgba(0,0,0,0.08)] backdrop-blur supports-backdrop-filter:bg-background/85">
          <div className="flex flex-wrap items-end gap-3">
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="areaManejo">Área do manejo</Label>
              <Select
                value={areaManejoId}
                onValueChange={(v) => setAreaManejoId(v ?? "")}
                items={areas.map((a) => ({ value: a.id, label: a.nome }))}
              >
                <SelectTrigger id="areaManejo" className="w-40">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <CampoMedicamentos
                ...
                selecionados={medicamentosSelecionados}
                onChange={setMedicamentosSelecionados}
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="responsavelManejo">Responsável</Label>
              <Input
                id="responsavelManejo"
                value={responsavel}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Input
                id="descricaoManejo"
                placeholder="Ex.: Vermifugação de outono"
                value={descricao}
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="observacoesManejo">Observações</Label>
              <Textarea
                id="observacoesManejo"
                value={observacoes}
            ...
            <Button
              type="button"
              disabled={emAndamento || selecionados.size === 0 || !areaManejoId}
              onClick={confirmarManejo}
            >
              {emAndamento ? "Registrando..." : `Registrar manejo (${selecionados.size})`}
            </Button>
          </div>
